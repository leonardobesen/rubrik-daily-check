...
def get_all_cluster_info(access_token: str) -> list[Cluster]:
    ...
    if not response["data"]["allClusterConnection"]["nodes"]:
        return []
    ...
    if response["data"]["snappableGroupByConnection"]["nodes"]:
        in_compliance, out_of_compliance = data_operation.process_compliance_information(response)
        for cluster in clusters_information:
            cluster_name_lower = cluster.name.lower()

            if cluster_name_lower in in_compliance:
                cluster.set_in_compliance_count(in_compliance[cluster_name_lower])
            
            if cluster_name_lower in out_of_compliance:
                cluster.set_out_of_compliance_count(out_of_compliance[cluster_name_lower])
